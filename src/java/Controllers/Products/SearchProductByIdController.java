 ...
 * @author Admin
 */
public class SearchProductByIdController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = homePage;
        String searchIdValue = request.getParameter("txtSearchIdValue");
        ProductsDAO productDAO = new ProductsDAO();
        CategoriesDAO categoryDAO = new CategoriesDAO();
        List<Categories> categoryList = categoryDAO.getAllCategories();
        List<Products2> productList = null;
        if(searchIdValue.isEmpty()){
            request.setAttribute("message", "Please enter Pizza's Id");
            productList = productDAO.getAllProducts2();
        }
        else{
            productList = productDAO.searchProductByProduct2Id(searchIdValue);
        }
